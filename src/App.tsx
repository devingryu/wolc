...
function App() {
  ...
  const [sendingWolDeviceId, setSendingWolDeviceId] = useState<string | null>(
    null
  );
  ...
  const handleSendWolForDevice = async (device: Device) => {
    ...
    try {
      ...
      showToast(
        t("toast.wol.sendSuccessTitle"),
        t("toast.wol.sendSuccessDescription", { mac: device.mac }),
        "default"
      );
      console.log(
        `WOL 전송 성공: MAC=${device.mac}, Target=${
          device.targetAddr || "Default"
        }, Port=${device.port ?? 9}`
      );
    } catch (error) {
  ...
  const handleModalFormChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "mac") {
      // 1. 입력값을 대문자로 변환하고 16진수 문자(0-9, A-F) 외에는 모두 제거
      const cleanedValue = value.toUpperCase().replace(/[^0-9A-F]/g, "");
      // 2. 최대 12자리까지만 사용 (MAC 주소는 12자리 16진수)
      const limitedValue = cleanedValue.substring(0, 12);
      // 3. 두 글자마다 콜론(:)을 삽입 (정규식 사용)
      //    - limitedValue.match(/.{1,2}/g) => 문자열을 1~2글자 단위로 잘라 배열 생성 (예: ["00", "11", "AA", "BB"])
      //    - ?.join(':') => 배열 요소들을 콜론으로 연결. 배열이 null이면(입력이 없으면) 빈 문자열 반환
      const formattedMac = limitedValue.match(/.{1,2}/g)?.join(":") || "";

      // 상태 업데이트 시 포맷된 값으로 설정
      setModalFormData((prev) => ({ ...prev, [name]: formattedMac }));
    } else {
      // 다른 필드는 기존 방식대로 처리
  ...
  const handleSaveDevice = async () => {
    ...
    if (!name || !mac) {
      showToast(
        t("toast.device.saveFailTitle"),
        t("toast.device.saveFailDescription"),
        "destructive"
      );
      return;
    }
    ...
    if (portStr) {
      ...
      if (!isNaN(parsedPort) && parsedPort > 0 && parsedPort <= 65535) {
        ...
        showToast(
          t("toast.device.saveFailTitle"),
          "유효한 포트 번호(1-65535)를 입력하거나 비워두세요.",
          "destructive"
        );
        return;
      }
    ...
    try {
      ...
      if (editingDevice) {
        updatedList = await invoke<Device[]>("update_device", {
          device: devicePayload,
        });
        showToast(
          t("toast.device.updateSuccessTitle"),
          t("toast.device.updateSuccessDescription", { name: name }),
          "default"
        );
      } else {
        updatedList = await invoke<Device[]>("add_device", {
          device: devicePayload,
        });
        showToast(
          t("toast.device.addSuccessTitle"),
          t("toast.device.addSuccessDescription", { name: name }),
          "default"
        );
      }
      ...
      handleError(
        error,
        editingDevice ? "errorContext.updateDevice" : "errorContext.addDevice"
      );
    } finally {
      setIsSavingDevice(false);
    }
  ...
  const handleDeleteDevice = async () => {
    ...
    try {
      const updatedList = await invoke<Device[]>("delete_device", {
        deviceId: deletingDevice.id,
      });
      setDevices(updatedList);
      showToast(
        t("toast.device.deleteSuccessTitle"),
        t("toast.device.deleteSuccessDescription", {
      ...
      <main>
        {isLoadingDevices ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-1">
            {devices.map((device) => {
              const targetDisplay = `${
                device.targetAddr || t("common.defaultValue")
              }:${device.port ?? 9}`;
              const targetTooltip = `${
                device.targetAddr || t("deviceCard.defaultTargetAddrTooltip")
              }, ${t("deviceCard.portLabel")}: ${device.port ?? 9}`;

              return (
                <Card
                  key={device.id}
                  className="flex flex-col overflow-hidden py-2 px-2"
                >
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger>
                          <Button
                            ...
                            size="icon"
                            className="h-8 w-8 flex-shrink-0"
                            aria-label={t("deviceCard.manageMenu")}
                            title={t("deviceCard.manageMenu")}
                          >
      ...
      <Dialog open={isAddEditModalOpen} onOpenChange={setIsAddEditModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>
              {editingDevice
                ? t("addEditDialog.title.edit")
                : t("addEditDialog.title.add")}
            </DialogTitle>
            <DialogDescription>
              {editingDevice
                ? t("addEditDialog.description.edit", {
                    name: editingDevice.name,
                  })
                : t("addEditDialog.description.add")}
            </DialogDescription>
          </DialogHeader>

          <div className="overflow-y-auto max-h-[60vh] pr-6 pl-1 py-4">
            <div className="grid gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="name">
                  {t("addEditDialog.label.name")}{" "}
              ...
              {/* 포트 필드 */}
              <div className="space-y-1.5">
                <Label htmlFor="port">{t("addEditDialog.label.port")}</Label>
                <Input
                  id="port"
                  name="port"
                  type="number"
          ...
          <DialogFooter className="flex-row justify-end space-x-2 pt-4">
            <DialogClose asChild>
              <Button type="button" variant="outline" disabled={isSavingDevice}>
                {t("common.cancel")}
              </Button>
            </DialogClose>
            <Button
